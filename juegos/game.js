...
const CLEF_GLYPH_SCALE = 20 / 250; // our staff line spacing is 20px

function renderClefGlyph(clefType) {
  const originX = 24;
  const originY = clefType === "sol" ? 120 : 80; // Sol line / Fa line
  const path = clefType === "sol" ? GCLEF_PATH : FCLEF_PATH;

  const outer = document.createElementNS("http://www.w3.org/2000/svg", "g");
  outer.setAttribute("class", "clef-glyph");
  outer.setAttribute("transform", `translate(${originX}, ${originY}) scale(${CLEF_GLYPH_SCALE})`);
  ...
  glyphPath.setAttribute("fill", "var(--color-gold)");
  glyphPath.setAttribute("d", path);

  flip.appendChild(glyphPath);
...
function drawStaffLines(clefType) {
  ...
  // Render Clef Symbol (real vector glyph, aligned to its reference line)
  staffSvg.appendChild(renderClefGlyph(clefType));
}

function drawNoteOnStaff(note, clef) {
  // Clear any existing note elements (keep lines and clefs)
  const linesAndClefs = Array.from(staffSvg.querySelectorAll("line, g.clef-glyph"));
  staffSvg.innerHTML = "";
